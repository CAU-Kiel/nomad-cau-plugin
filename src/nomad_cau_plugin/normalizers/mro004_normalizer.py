...
class MRO004Normalizer:
    ...
    def process_csv_data(archive, data_file, logger):
        ...
        with archive.m_context.raw_file(data_file, 'rb') as file:
            ...
            try:
                df = pd.read_csv(file, skiprows=[1], decimal=',', sep=r'\t|;', encoding='utf-8')
                logger.info(f"Successfully read CSV file with UTF-8 encoding")
            except UnicodeDecodeError:
                # If UTF-8 fails, try with latin-1 encoding (common for German files)
                file.seek(0)  # Reset file pointer
                try:
                    df = pd.read_csv(file, skiprows=[1], decimal=',', sep=r'\t|;', encoding='latin-1')
                    logger.info(f"Successfully read CSV file with latin-1 encoding")
                except Exception as e:
                    logger.error(f"Failed to read CSV file with both UTF-8 and latin-1 encodings: {e}")
                    raise
        ...
        try:
            with archive.m_context.raw_file(report_file, 'rb') as file:
                ...
                try:
                    ...
                    chemistry_df, setup_df, recipe_df = extract_tables_from_report(tmp_file_path)
                    ...
                    if not chemistry_df.empty:
                        ...
                        for row in extracted_rows:
                            ...
                            try:
                                mol_weight_value = float(mol_weight_str.split()[0])
                                chemical.mol_weight = ureg.Quantity(mol_weight_value, 'g/mol')
                            except:
                                chemical.mol_weight = ureg.Quantity(0, 'g/mol')
                            
                            # Parse actual moles
                            actual_moles_str = row['Actual Moles']
                            try:
                                actual_moles_value = float(actual_moles_str.split()[0])
                                chemical.actual_moles = ureg.Quantity(actual_moles_value, 'mol')
                            except:
                                chemical.actual_moles = ureg.Quantity(0, 'mol')
                            
                            # Parse actual amount
                            actual_amount_str = row['Actual Amount']
                            try:
                                actual_amount_value = float(actual_amount_str.split()[0])
                                chemical.actual_amount = ureg.Quantity(actual_amount_value, 'g')
                            except:
                                chemical.actual_amount = ureg.Quantity(0, 'g')
                            
                            # Parse concentration
                            concentration_str = row['Concentration']
                            try:
                                concentration_value = concentration_str.split()[0]
                                chemical.concentration = concentration_value
                            except:
